...
import libs2p.rpc_model as rpc_model
...
def rpc_from_geotiff(geotiff_path, outrpcfile='.rpc'):
    ...
    # Initialize the rpc model
    rpc = rpc_model.RPCModel(outrpcfile)
    
    # Remove the temporary rpc file
    os.remove(outrpcfile)
    return rpc
...
def lonlat_from_utm(easts, norths, zonestring):
    ...
    latpz  = re.sub(r'\d+', '', zonestring)
    longpz = int(zonestring[:-1])
