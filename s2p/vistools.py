...
def display_imshow2(im, cmap='gray', subtitle='image'):
    import matplotlib.pyplot as plt

    plt.figure(figsize=(10, 5))
    #plt.suptitle('Result Image')

    #print('show the original images')
    #plt.subplot(1, 1, 1) , plt.title(subtitle)
    plt.imshow(im , cmap), plt.axis('off')
    plt.colorbar()

    plt.show()


def display_imshow3(im, cmap='gray', subtitle='image'):
    import matplotlib.pyplot as plt
    ...
    im_x = im.shape[1]
    im_y = im.shape[0]
    X1 = np.arange(0,im_x,1)
    Y1 = np.arange(0,im_y,1)
    X1,Y1 = np.meshgrid(X1,Y1)
    Z1 = im/10
    min_number1 = np.min(Z1)
    max_number1 = np.max(Z1)
    print(min_number1)
    print(max_number1)

    ax.plot_surface(X1,Y1,Z1,rstride=1,cstride=1,cmap='jet')
    ax.contourf(X1,Y1,Z1,zdir='z',offset=-2,cmap='jet')
...
### next functions wrote by mchen

def display_overlap(im,xy_h):
    import matplotlib.pyplot as plt
    ...
    for i in range(row):
        for j in range(col):
            ...
            if left>=0:
                if top>=0:
                    if xy_h[top][left]==xy_h[i][j]:
                        flag = flag +1
                if xy_h[i][left]==xy_h[i][j]:
                    flag = flag +1
                if bottom<row:
                    if xy_h[bottom][left]==xy_h[i][j]:
                        flag = flag +1
            if right<col:
                if top>=0:
                    if xy_h[top][right]==xy_h[i][j]:
                        flag = flag +1
                if xy_h[i][right]==xy_h[i][j]:
                    flag = flag +1
                if bottom<row:
                    if xy_h[bottom][right]==xy_h[i][j]:
                        flag = flag +1
            if top>=0:
                if xy_h[top][j]==xy_h[i][j]:
                    flag = flag +1
            if bottom<row:
                if xy_h[bottom][j]==xy_h[i][j]:
                    flag = flag +1
    ...
    plt.figure(figsize=(10, 5))
    plt.imshow(im , "jet"), plt.axis('off')
    plt.colorbar()
    plt.show()



def display_hist(data,count):
    import matplotlib.pyplot as plt
...
### display in pycharm
def display_result_show(ori_1,ori_2,rec_1,rec_2,disp,cloud,dem):
    '''
        display the input, the intermediate results and the output using matplotlib

    Arguments:
        all of them are numpy array
        ori_1,ori_2: original map
        rec_1,rec_2: rectified map
        disp: disparity map
    ...
    print('show the original images')
    plt.subplot(3, 2, 1), plt.title('original-1')
    plt.imshow(ori_1 / 8, cmap='gray'), plt.axis('off')
    plt.colorbar()
    plt.subplot(3, 2, 2), plt.title('original-2')
    plt.imshow(ori_2 / 8, cmap='gray'), plt.axis('off')
    plt.colorbar()

    print('show the rectified images')
    plt.subplot(3, 2, 3), plt.title('rectified-1')
    plt.imshow(rec_1 / 8, cmap='gray'), plt.axis('off')
    plt.colorbar()
    plt.subplot(3, 2, 4), plt.title('rectified-2')
    plt.imshow(rec_2 / 8, cmap='gray'), plt.axis('off')
    plt.colorbar()

    print('show the disparity map')
    plt.subplot(3, 2, 5), plt.title('disparities')
    plt.imshow(disp, cmap='jet'), plt.axis('off')
    plt.colorbar()

    # print('show the point cloud')
    #display(display_cloud(cloud))

    print('show the DEM')
    plt.subplot(3, 2, 6), plt.title('DEM')
    plt.imshow(dem, cmap='jet'), plt.axis('off')
    plt.colorbar()
...
def save_tiff(im, imDir='./save/image.tif'):
    #from scipy import misc
    #misc.imsave(imDir, im)
    import imageio
    imageio.imwrite(imDir, im)
